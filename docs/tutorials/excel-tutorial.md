...
## <a name="create-a-table"></a><span data-ttu-id="7b6a8-125">Créer un tableau</span><span class="sxs-lookup"><span data-stu-id="7b6a8-125">Create a table</span></span>
...
10. <span data-ttu-id="7b6a8-p111">Remplacez `TODO6` par le code suivant. Remarque :</span><span class="sxs-lookup"><span data-stu-id="7b6a8-p111">Replace `TODO6` with the following code. Note:</span></span>
...
3. <span data-ttu-id="7b6a8-171">Exécutez la commande `npm start` pour démarrer un serveur web en cours d’exécution sur localhost.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-171">Run the command `npm start` to start a web server running on localhost.</span></span>
...
### <a name="filter-the-table"></a><span data-ttu-id="7b6a8-181">Filtrage du tableau</span><span class="sxs-lookup"><span data-stu-id="7b6a8-181">Filter the table</span></span>
...
### <a name="test-the-add-in"></a><span data-ttu-id="7b6a8-207">Test du complément</span><span class="sxs-lookup"><span data-stu-id="7b6a8-207">Test the add-in</span></span>
...
5. <span data-ttu-id="7b6a8-227">En dessous de la ligne qui attribue un gestionnaire de clic au bouton `sort-chart`, ajoutez le code suivant :</span><span class="sxs-lookup"><span data-stu-id="7b6a8-227">Below the line that assigns a click handler to the `sort-chart` button, add the following code:</span></span>
...
8. <span data-ttu-id="7b6a8-231">Remplacez `TODO2` par le code suivant.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-231">Replace `TODO2` with the following code.</span></span> <span data-ttu-id="7b6a8-232">Notez les paramètres suivants :</span><span class="sxs-lookup"><span data-stu-id="7b6a8-232">Note the following parameters:</span></span>
   ...
   - <span data-ttu-id="7b6a8-235">Le deuxième paramètre spécifie la plage de données à inclure dans le graphique.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-235">The second parameter specifies the range of data to include in the chart.</span></span>
...
### <a name="test-the-add-in"></a><span data-ttu-id="7b6a8-248">Test du complément</span><span class="sxs-lookup"><span data-stu-id="7b6a8-248">Test the add-in</span></span>
...
### <a name="freeze-the-tables-header-row"></a><span data-ttu-id="7b6a8-266">Figer la ligne d’en-tête du tableau</span><span class="sxs-lookup"><span data-stu-id="7b6a8-266">Freeze the table's header row</span></span>
...
3. <span data-ttu-id="7b6a8-286">Exécutez la commande `npm start` pour démarrer un serveur web en cours d’exécution sur localhost.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-286">Run the command `npm start` to start a web server running on localhost.</span></span>
...
1. <span data-ttu-id="7b6a8-296">Ouvrez le fichier manifeste my-office-add-in-manifest.xml.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-296">Open the manifest file my-office-add-in-manifest.xml.</span></span>
...
2. <span data-ttu-id="7b6a8-356">Il n’est pas possible que deux instructions `return` se trouvent dans le même chemin de code, donc supprimez la dernière ligne `return context.sync();` à la fin de la fonction `Excel.run`.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-356">You can't have two `return` statements in the same unbranching code path, so delete the final line `return context.sync();` at the end of the `Excel.run`.</span></span> <span data-ttu-id="7b6a8-357">Vous ajouterez un nouvel élément final `context.sync` dans une étape ultérieure.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-357">You will add a new final `context.sync`, in a later step.</span></span>
...
### <a name="configure-the-script-loading-html-file"></a><span data-ttu-id="7b6a8-364">Configuration du fichier HTML de chargement de script</span><span class="sxs-lookup"><span data-stu-id="7b6a8-364">Configure the script-loading HTML file</span></span>
...
2. <span data-ttu-id="7b6a8-376">Supprimez le cache Office en supprimant le contenu du dossier de cache.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-376">Delete the Office cache by deleting the contents of the cache folder.</span></span> <span data-ttu-id="7b6a8-377">Cette opération est nécessaire pour effacer complètement de l’hôte l’ancienne version du complément.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-377">This is necessary to completely clear the old version of the add-in from the host.</span></span> 
    ...
    - <span data-ttu-id="7b6a8-378">Pour Windows : `%LOCALAPPDATA%\Microsoft\Office\16.0\Wef\`.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-378">For Windows: `%LOCALAPPDATA%\Microsoft\Office\16.0\Wef\`.</span></span>
...
3. <span data-ttu-id="7b6a8-p156">Ajoutez le balisage suivant au fichier popup.html. Remarque :</span><span class="sxs-lookup"><span data-stu-id="7b6a8-p156">Add the following markup to popup.html. Note:</span></span>

   - <span data-ttu-id="7b6a8-402">La page comporte un champ `<input>`, dans lequel l’utilisateur entrera son nom, et un bouton qui permet d’envoyer le nom à la page dans le volet Office où il sera affiché.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-402">The page has a `<input>` where the user will enter their name and a button that will send the name to the page in the task pane where it will be displayed.</span></span>

   - <span data-ttu-id="7b6a8-403">Le balisage charge un script appelé popup.js que vous allez créer dans une étape ultérieure.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-403">The markup loads a script called popup.js that you will create in a later step.</span></span>

   - <span data-ttu-id="7b6a8-404">Il charge également la bibliothèque Office.JS et jQuery, car ils seront utilisés dans popup.js.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-404">It also loads the Office.JS library and jQuery because they will be used in popup.js.</span></span>
...
4. <span data-ttu-id="7b6a8-405">Créez un fichier à la racine du projet et nommez-le popup.js.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-405">Create a file in the root of the project called popup.js.</span></span>

5. <span data-ttu-id="7b6a8-406">Ajoutez le code suivant au fichier popup.js.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-406">Add the following code to popup.js.</span></span> <span data-ttu-id="7b6a8-407">Tenez compte des informations suivantes :</span><span class="sxs-lookup"><span data-stu-id="7b6a8-407">Note the following about this code:</span></span>

   - <span data-ttu-id="7b6a8-408">*Toutes les pages appellent les API dans la bibliothèque Office.JS doivent tout d’abord vérifier que la bibliothèque est entièrement initialisée.*</span><span class="sxs-lookup"><span data-stu-id="7b6a8-408">*Every page that calls APIs in the Office.JS library must first ensure that the library is fully initialized.*</span></span> <span data-ttu-id="7b6a8-409">La meilleure façon de procéder consiste à appeler la méthode `Office.onReady()`.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-409">The best way to do that is to call the `Office.onReady()` method.</span></span> <span data-ttu-id="7b6a8-410">Si votre complément dispose de ses propres tâches d’initialisation, le code doit passer dans une méthode `then()` chaînée à l’appel de `Office.onReady()`.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-410">If your add-in has its own initialization tasks, the code should go in a `then()` method that is chained to the call of `Office.onReady()`.</span></span> <span data-ttu-id="7b6a8-411">Pour voir un exemple, affichez le fichier app.js à la racine du projet.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-411">For an example, see the app.js file in the project root.</span></span> <span data-ttu-id="7b6a8-412">Le code qui appelle `Office.onReady()` doit être exécuté avant tout appel à Office.JS ; l’affectation se trouve donc dans un fichier de script chargé par la page, comme dans ce cas.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-412">The call of `Office.onReady()` must run before any calls to Office.JS; hence the assignment is in a script file that is loaded by the page, as it is in this case.</span></span>
   - <span data-ttu-id="7b6a8-413">La fonction `ready` jQuery est appelée à l’intérieur de la méthode `then()`.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-413">The jQuery `ready` function is called inside the `then()` method.</span></span> <span data-ttu-id="7b6a8-414">Dans la plupart des cas, le code de chargement, d’initialisation ou d’amorçage d’autres bibliothèques JavaScript se trouve à l’intérieur de la méthode `then()` chaînée à l’appel de `Office.onReady()`.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-414">In most cases, the loading, initializing, or bootstrapping code of other JavaScript libraries should be inside the `then()` method that is chained to the call of `Office.onReady()`.</span></span>
...
6. <span data-ttu-id="7b6a8-415">Remplacez `TODO1` par le code suivant.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-415">Replace `TODO1` with the following code.</span></span> <span data-ttu-id="7b6a8-416">Vous allez créer la fonction `sendStringToParentPage` à l’étape suivante.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-416">You'll create the `sendStringToParentPage` function in the next step.</span></span>

    ```js
    $('#ok-button').click(sendStringToParentPage);
    ```

7. <span data-ttu-id="7b6a8-417">Remplacez `TODO2` par le code suivant.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-417">Replace `TODO2` with the following code.</span></span> <span data-ttu-id="7b6a8-418">La méthode `messageParent` transmet son paramètre à la page parent, qui est, dans ce cas, la page dans le volet Office.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-418">The `messageParent` method passes its parameter to the parent page, in this case, the page in the task pane.</span></span> <span data-ttu-id="7b6a8-419">Le paramètre peut être une valeur booléenne ou une chaîne qui inclut tous les éléments qui peuvent être sérialisés en tant que chaîne, au format XML ou JSON.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-419">The parameter can be a boolean or a string, which includes anything that can be serialized as a string, such as XML or JSON.</span></span>
...
8. <span data-ttu-id="7b6a8-420">Enregistrez le fichier.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-420">Save the file.</span></span>

   > [!NOTE]
   > <span data-ttu-id="7b6a8-421">Le fichier popup.html et le fichier popup.js qu’il charge s’exécutent dans un processus Internet Explorer entièrement séparé à partir du volet Office du complément.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-421">The popup.html file, and the popup.js file that it loads, run in an entirely separate Internet Explorer process from the add-in's task pane.</span></span> <span data-ttu-id="7b6a8-422">Si le popup.js était transpilé dans le même fichier bundle.js en tant que fichier app.js, le complément devrait charger deux copies du fichier bundle.js, ce qui irait à l’encontre de l’objectif de groupement.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-422">If the popup.js was transpiled into the same bundle.js file as the app.js file, then the add-in would have to load two copies of the bundle.js file, which defeats the purpose of bundling.</span></span> <span data-ttu-id="7b6a8-423">En outre, le fichier popup.js ne contient pas de code JavaScript car Internet Explorer ne prend pas en charge ce type de code.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-423">In addition, the popup.js file does not contain any JavaScript that is unsupported by IE.</span></span> <span data-ttu-id="7b6a8-424">C’est pour ces deux raisons que ce complément ne transpile pas le fichier popup.js du tout.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-424">For these two reasons, this add-in does not transpile the popup.js file at all.</span></span>

### <a name="open-the-dialog-from-the-task-pane"></a><span data-ttu-id="7b6a8-425">Ouverture de la boîte de dialogue à partir du volet Office</span><span class="sxs-lookup"><span data-stu-id="7b6a8-425">Open the dialog from the task pane</span></span>

1. <span data-ttu-id="7b6a8-426">Ouvrez le fichier index.html.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-426">Open the file index.html.</span></span>

2. <span data-ttu-id="7b6a8-427">Sous la balise `div` qui contient le bouton `freeze-header`, ajoutez le balisage suivant :</span><span class="sxs-lookup"><span data-stu-id="7b6a8-427">Below the `div` that contains the `freeze-header` button, add the following markup:</span></span>
...
3. <span data-ttu-id="7b6a8-428">La boîte de dialogue invitera l’utilisateur à saisir son nom et transmettra ce nom au volet Office.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-428">The dialog will prompt the user to enter a name and pass the user's name to the task pane.</span></span> <span data-ttu-id="7b6a8-429">Le volet Office s’affichera dans une étiquette.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-429">The task pane will display it in a label.</span></span> <span data-ttu-id="7b6a8-430">Juste en dessous de la balise `div` que vous venez d’ajouter, ajoutez le balisage suivant :</span><span class="sxs-lookup"><span data-stu-id="7b6a8-430">Immediately below the `div` that you just added, add the following markup:</span></span>
...
4. <span data-ttu-id="7b6a8-431">Ouvrez le fichier app.js.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-431">Open the app.js file.</span></span>

5. <span data-ttu-id="7b6a8-432">Sous la ligne qui attribue un gestionnaire de clics au bouton `freeze-header`, ajoutez le code suivant.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-432">Below the line that assigns a click handler to the `freeze-header` button, add the following code.</span></span> <span data-ttu-id="7b6a8-433">Vous allez créer la méthode `openDialog` à une étape ultérieure.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-433">You'll create the `openDialog` method in a later step.</span></span>

    ```js
    $('#open-dialog').click(openDialog);
    ```

6. <span data-ttu-id="7b6a8-p165">Ajoutez la déclaration suivante sous la fonction `freezeHeader`. Cette variable est utilisée pour conserver un objet dans le contexte d’exécution de la page parent qui agit en tant qu’intermédiaire pour le contexte d’exécution de la page de boîte de dialogue.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-p165">Below the `freezeHeader` function add the following declaration. This variable is used to hold an object in the parent page's execution context that acts as an intermediator to the dialog page's execution context.</span></span>

    ```js
    var dialog = null;
    ```

7. <span data-ttu-id="7b6a8-436">Sous la déclaration de la balise `dialog`, ajoutez la fonction suivante.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-436">Below the declaration of `dialog`, add the following function.</span></span> <span data-ttu-id="7b6a8-437">Le plus important à remarquer à propos de ce code est ce qui ne s’y trouve *pas* : il n’y a aucun appel de `Excel.run`.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-437">The important thing to notice about this code is what is *not* there: there is no call of `Excel.run`.</span></span> <span data-ttu-id="7b6a8-438">Cela est dû au fait que l’API d’ouverture de boîte de dialogue est partagée par tous les hôtes Office, elle fait donc partie de l’API commune JavaScript Office, pas de l’API spécifique d’Excel.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-438">This is because the API to open a dialog is shared among all Office hosts, so it is part of the Office JavaScript Common API, not the Excel-specific API.</span></span>
...
8. <span data-ttu-id="7b6a8-p167">Remplacez `TODO1` par le code suivant. Tenez compte des informations suivantes :</span><span class="sxs-lookup"><span data-stu-id="7b6a8-p167">Replace `TODO1` with the following code. Note:</span></span>

   - <span data-ttu-id="7b6a8-441">La méthode `displayDialogAsync` ouvre une boîte de dialogue au centre de l’écran.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-441">The `displayDialogAsync` method opens a dialog in the center of the screen.</span></span>

   - <span data-ttu-id="7b6a8-442">Le premier paramètre est l’URL de la page à ouvrir.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-442">The first parameter is the URL of the page to open.</span></span>

   - <span data-ttu-id="7b6a8-p168">Le deuxième paramètre transmet les options. `height` et `width` sont des pourcentages de la taille de la fenêtre de l’application Office.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-p168">The second parameter passes options. `height` and `width` are percentages of the size of the Office application's window.</span></span>
...
### <a name="process-the-message-from-the-dialog-and-close-the-dialog"></a><span data-ttu-id="7b6a8-445">Traitement du message à partir de la boîte de dialogue et fermeture de la boîte de dialogue</span><span class="sxs-lookup"><span data-stu-id="7b6a8-445">Process the message from the dialog and close the dialog</span></span>

1. <span data-ttu-id="7b6a8-p169">Continuez dans le fichier app.js et remplacez `TODO2` par le code suivant. Remarque :</span><span class="sxs-lookup"><span data-stu-id="7b6a8-p169">Continue in the app.js file, and replace `TODO2` with the following code. Note:</span></span>

   - <span data-ttu-id="7b6a8-448">Le rappel est exécuté immédiatement après que la boîte de dialogue s’est ouverte correctement et avant que l’utilisateur ait pris une quelconque action dans la boîte de dialogue.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-448">The callback is executed immediately after the dialog successfully opens and before the user has taken any action in the dialog.</span></span>

   - <span data-ttu-id="7b6a8-449">`result.value` représente l’objet qui agit comme un intermédiaire entre les contextes d’exécution des pages parent et de boîte de dialogue.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-449">The `result.value` is the object that acts as a kind of middleman between the execution contexts of the parent and dialog pages.</span></span>

   - <span data-ttu-id="7b6a8-450">La fonction `processMessage` sera créée à une étape ultérieure.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-450">The `processMessage` function will be created in a later step.</span></span> <span data-ttu-id="7b6a8-451">Ce gestionnaire traitera toutes les valeurs envoyées par la page de boîte de dialogue avec les appels de la fonction `messageParent`.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-451">This handler will process any values that are sent from the dialog page with calls of the `messageParent` function.</span></span>
...
2. <span data-ttu-id="7b6a8-452">Sous la fonction `openDialog`, ajoutez la fonction suivante.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-452">Below the `openDialog` function, add the following function.</span></span>
...
### <a name="test-the-add-in"></a><span data-ttu-id="7b6a8-453">Test du complément</span><span class="sxs-lookup"><span data-stu-id="7b6a8-453">Test the add-in</span></span>

1. <span data-ttu-id="7b6a8-454">Si la fenêtre Git Bash, ou l’invite système Node.JS, de l’étape précédente du didacticiel est encore ouverte, appuyez sur \*\*Ctrl+C \*\*à deux reprises pour arrêter le serveur web en cours d’exécution.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-454">If the Git bash window, or Node.JS-enabled system prompt, from the previous stage tutorial is still open, enter **Ctrl+C** twice to stop the running web server.</span></span> <span data-ttu-id="7b6a8-455">Sinon, ouvrez une fenêtre Git Bash, ou une invite système Node.JS, et accédez au dossier **Start** du projet.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-455">Otherwise, open a Git bash window, or Node.JS-enabled system prompt, and navigate to the **Start** folder of the project.</span></span>

     > [!NOTE]
     > <span data-ttu-id="7b6a8-456">Bien que le serveur synchronisé au navigateur recharge votre complément dans le volet Office chaque fois que vous apportez une modification à un fichier, y compris le fichier app.js, il ne retranspile pas le code JavaScript. Vous devez donc de nouveau utiliser la commande build afin que les modifications apportées à app.js prennent effet.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-456">Although the browser-sync server reloads your add-in in the task pane every time you make a change to any file, including the app.js file, it does not retranspile the JavaScript, so you must repeat the build command in order for your changes to app.js to take effect.</span></span> <span data-ttu-id="7b6a8-457">Pour ce faire, vous devez arrêter le processus du serveur pour pouvoir obtenir une invite et saisir la commande build.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-457">In order to do this, you need to kill the server process in so that you can get a prompt to enter the build command.</span></span> <span data-ttu-id="7b6a8-458">Une fois la commande build exécutée, redémarrez le serveur.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-458">After the build, you restart the server.</span></span> <span data-ttu-id="7b6a8-459">Les prochaines étapes vous permettent d’effectuer ce processus.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-459">The next few steps carry out this process.</span></span>

2. <span data-ttu-id="7b6a8-460">Exécutez la commande `npm run build` pour transpiler votre code source ES6 vers une version antérieure de JavaScript prise en charge par Internet Explorer (qui est utilisé en arrière-plan par Excel pour exécuter les compléments Excel).</span><span class="sxs-lookup"><span data-stu-id="7b6a8-460">Run the command `npm run build` to transpile your ES6 source code to an earlier version of JavaScript that is supported by Internet Explorer (which is used under-the-hood by Excel to run Excel add-ins).</span></span>

3. <span data-ttu-id="7b6a8-461">Exécutez la commande `npm start` pour démarrer un serveur web en cours d’exécution sur localhost.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-461">Run the command `npm start` to start a web server running on localhost.</span></span>

4. <span data-ttu-id="7b6a8-462">Recharger le volet Office en le fermant, puis, dans le menu **Accueil**, sélectionnez **Afficher le volet des pages** pour rouvrir le complément.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-462">Reload the task pane by closing it, and then on the **Home** menu, select **Show Taskpane** to reopen the add-in.</span></span>

5. <span data-ttu-id="7b6a8-463">Sélectionnez le bouton **Boîte de dialogue Ouvrir** dans le volet Office.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-463">Choose the **Open Dialog** button in the task pane.</span></span>

6. <span data-ttu-id="7b6a8-464">Lorsque la boîte de dialogue est ouverte, faites-la glisser et redimensionnez-la.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-464">While the dialog is open, drag it and resize it.</span></span> <span data-ttu-id="7b6a8-465">Notez que vous pouvez interagir avec la feuille de calcul, appuyez sur les autres boutons dans le volet Office, mais vous ne pouvez pas lancer une deuxième boîte de dialogue à partir de la même page de volet de tâches.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-465">Note that you can interact with the worksheet and press other buttons on the task pane, but you cannot launch a second dialog from the same task pane page.</span></span>

7. <span data-ttu-id="7b6a8-466">Dans la boîte de dialogue, entrez un nom et appuyez sur **OK**.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-466">In the dialog, enter a name and choose **OK**.</span></span> <span data-ttu-id="7b6a8-467">Ce nom apparaît sur le volet Office et la boîte de dialogue se ferme.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-467">The name appears on the task pane and the dialog closes.</span></span>

8. <span data-ttu-id="7b6a8-468">Si vous le souhaitez, vous pouvez commenter la ligne `dialog.close();` dans la fonction `processMessage`.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-468">Optionally, comment out the line `dialog.close();` in the `processMessage` function.</span></span> <span data-ttu-id="7b6a8-469">Ensuite, répétez les étapes de cette section.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-469">Then repeat the steps of this section.</span></span> <span data-ttu-id="7b6a8-470">La boîte de dialogue reste ouverte et vous pouvez modifier le nom.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-470">The dialog stays open and you can change the name.</span></span> <span data-ttu-id="7b6a8-471">Vous pouvez la fermer manuellement en appuyant sur la croix (**X**) en haut à droite.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-471">You can close it manually by pressing the **X** button in the upper right corner.</span></span>

    ![Didacticiel Excel- Boîte de dialogue](../images/excel-tutorial-dialog-open.png)

## <a name="next-steps"></a><span data-ttu-id="7b6a8-473">Étapes suivantes</span><span class="sxs-lookup"><span data-stu-id="7b6a8-473">Next steps</span></span>

<span data-ttu-id="7b6a8-474">Ce didacticiel vous apprend à créer un complément Excel qui interagit avec des tableaux, des graphiques (chart), des feuilles de calcul et des boîtes de dialogue dans un classeur Excel.</span><span class="sxs-lookup"><span data-stu-id="7b6a8-474">In this tutorial, you've created an Excel task pane add-in that interacts with tables, charts, worksheets, and dialogs in an Excel workbook.</span></span> <span data-ttu-id="7b6a8-475">Pour en savoir plus sur le développement des complément Excel, passez à l’article suivant :</span><span class="sxs-lookup"><span data-stu-id="7b6a8-475">To learn more about building Excel add-ins, continue to the following article:</span></span>

> [!div class="nextstepaction"]
> [<span data-ttu-id="7b6a8-476">Présentation des compléments Excel</span><span class="sxs-lookup"><span data-stu-id="7b6a8-476">Excel add-ins overview</span></span>](../excel/excel-add-ins-overview.md)

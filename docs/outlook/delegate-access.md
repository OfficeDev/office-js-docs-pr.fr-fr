...
[L’objet DelegatePermissions est](/javascript/api/outlook/office.mailboxenums.delegatepermissions) implémenté à l’aide d’un masque de bits pour indiquer les autorisations. Chaque position dans le masque de bits représente une autorisation particulière et si elle est définie sur, l’utilisateur `1` dispose de l’autorisation respective. Par exemple, si le deuxième bit à partir de la droite `1`est , l’utilisateur dispose de **l’autorisation d’écriture** . Vous pouvez voir un exemple de vérification d’une autorisation spécifique dans [la section Effectuer](#perform-an-operation-as-delegate-or-shared-mailbox-user) une opération en tant que délégué ou utilisateur de boîte aux lettres partagée plus loin dans cet article.
...
Si vous souhaitez appeler une opération REST sur un élément, que l’élément soit partagé ou non, `getSharedPropertiesAsync` vous pouvez utiliser l’API pour déterminer si l’élément est partagé. Après cela, vous pouvez construire l’URL REST pour l’opération à l’aide de l’objet approprié.
...
En mode composition de message, [getSharedPropertiesAsync](/javascript/api/outlook/office.messagecompose#outlook-office-messagecompose-getsharedpropertiesasync-member(1)) n’est pas pris en charge dans Outlook sur le web ou sur Windows à moins que les conditions suivantes ne soient remplies.
...
Votre application peut utiliser REST et son autorisation doit être définie pour activer l’accès `ReadWriteMailbox` REST à la boîte aux lettres du propriétaire ou à la boîte aux lettres partagée, le cas échéant. EWS n’est pas pris en charge.
...
Si un administrateur a caché un utilisateur ou une adresse de boîte aux lettres partagée à partir d’une liste d’adresses telle que la liste d’adresses globale , `Office.context.mailbox.item` les éléments de courrier affectés ouverts dans le rapport de boîte aux lettres sont null. Par exemple, si l’utilisateur ouvre un élément de courrier dans une boîte aux lettres partagée qui est masquée dans la liste d’adresses gal, `Office.context.mailbox.item` représentant cet élément de courrier est null.

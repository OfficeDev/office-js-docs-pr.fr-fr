...
Par exemple, vous pouvez définir en `options.asyncContext` tant qu’objet JSON qui contient une ou plusieurs paires clé-valeur. Vous trouverez d’autres exemples sur la transmission de paramètres facultatifs à des méthodes asynchrones dans la plateforme de Office Add-ins dans la programmation [asynchrone dans les](../develop/asynchronous-programming-in-office-add-ins.md#pass-optional-parameters-to-asynchronous-methods) Office de recherche. L’exemple suivant montre comment utiliser le paramètre `asyncContext` pour passer 2 arguments à une méthode de rappel.
...
Similaire à `addFileAttachmentAsync`la méthode , `addItemAttachmentAsync`et `getAttachmentsAsync` aux méthodes, `removeAttachmentAsync` est une méthode asynchrone. Vous devez fournir une méthode de rappel pour vérifier `AsyncResult` l’état et toute erreur à l’aide de l’objet paramètre de sortie. Vous pouvez également transmettre des paramètres supplémentaires à la méthode de rappel à l’aide du paramètre `asyncContext` facultatif.

La fonction JavaScript suivante, `removeAttachment`continue d’étendre les exemples ci-dessus et supprime la pièce jointe spécifiée de l’e-mail ou du rendez-vous en cours de composition. La fonction prend comme argument l’ID de la pièce jointe à supprimer. Vous pouvez obtenir l’ID d’une `addFileAttachmentAsync``addFileAttachmentFromBase64Async`pièce jointe après un appel réussi, ou `addItemAttachmentAsync` de méthode, et l’utiliser dans un appel de `removeAttachmentAsync` méthode ultérieur. Vous pouvez également appeler `getAttachmentsAsync` (introduit dans l’ensemble de conditions requises 1.8) pour obtenir les pièces jointes et leurs ID pour cette session de module.

...
* Dans le fichier JavaScript, spécifiez les noms de fonction en CamelCase. Par exemple, le nom de la fonction `addTenToInput` est écrit en CamelCase : le premier mot dans le nom commence par une lettre minuscule, et chaque mot suivant dans le nom commence par une lettre majuscule.

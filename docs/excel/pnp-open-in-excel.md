...
Réduisez le nombre d’étapes en un seul clic sur votre page web qui génère et ouvre Excel document. Vous pouvez également incorporer votre Office à l’intérieur du document et l’afficher à l’ouverture du document. Cela garantit que le client a toujours accès aux fonctionnalités de votre application. Lorsque le document s’ouvre, les données que le client a sélectionnées et votre Office est déjà disponible pour qu’il continue de fonctionner.
...
Tout d’abord, nous allons apprendre à créer un document Excel à partir d’une page web et à incorporer un add-in dans le document. L Office exemple de code de l’incorporation [ooXML](https://github.com/OfficeDev/Office-OOXML-EmbedAddin) dans un document Script Lab dans un nouveau document [](https://appsource.microsoft.com/product/office/wa104380862) Office. Bien que l’exemple fonctionne avec Office document, nous nous concentrerons simplement sur Excel feuilles de calcul dans cet article. Utilisez les étapes suivantes pour créer et exécuter l’exemple.

1. Extrayez l’exemple de  https://github.com/OfficeDev/Office-OOXML-EmbedAddin/archive/master.zip code dans un dossier de votre ordinateur.
...
Fichier **OOXMLHelper.cs** :

- Fournit toutes les manipulations OOXML détaillées.
- Utilise une technique standard pour valider le Office, qui consiste simplement à appeler la **méthode Document.Open** dessus. Si le fichier n’est pas valide, la méthode envoie une exception.
- Contient principalement du code généré par les outils de productivité du SDK Open XML 2.5 disponibles dans le lien du [SDK Open XML 2.5](/office/open-xml/open-xml-sdk).
...
- La **valeur du Store** est l’URL du partage réseau . par exemple, «\\\\ MyComputerMySharedFolder\\ ». Il doit s’agit de l’URL exacte qui apparaît en tant qu’adresse de catalogue approuvé du partage dans Office de confiance.
...
:::image type="content" source="../images/fluent-ui-wxp.png" alt-text="Fluent’interface utilisateur pour Word, Excel et PowerPoint.":::
...
1. Suivez les [https://developer.microsoft.com/graph/quick-start](https://developer.microsoft.com/graph/quick-start) étapes de création et d’ouverture d’un exemple de code de démarrage rapide qui interagit avec Office services.
...
1. À **l’étape 2 : Obtenez un ID d’application et une secret**, sélectionnez Obtenir **un ID d’application et un secret**.
...
1. À **l’étape 3 : Commencez à coder**, choisissez **Télécharger l’exemple de code basé sur le SDK**.
...
1. Ouvrez **la solution graph-tutorial.sln** Visual Studio 2019 et ouvrez **lePrivateSettings.config** fichier.

1. Ajoutez une nouvelle **étendue Files.ReadWriteto**  à la clé **ida:AppScopes** afin qu’elle ressemble au code suivant.
...
- **Passez une propriété de requête personnalisée à votre Office de recherche**. Lorsque vous générez le document, vous incorporez une propriété personnalisée pour le Office qui contient une chaîne de requête qui récupère toutes les données requises. Lorsque votre application s’ouvre, elle récupère la requête, l’exécute et utilise l’API JS Office pour insérer le résultat de la requête dans le document.

...
Ce tableau croisé dynamique peut être généré par le biais de l'API JavaScript ou de l'interface utilisateur d'Excel. Les deux options permettent d'autres manipulations par le biais de leurs add-ins.
...
> [!NOTE]
> Certaines des fonctionnalités PivotLayout mentionnées ici sont actuellement disponibles uniquement en prévisualisation publique. [!INCLUDE [Information about using preview APIs](../includes/using-excel-preview-apis.md)]

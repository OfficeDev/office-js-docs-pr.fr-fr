...
| `id`     | string | Oui | ID unique de la fonction. Ce code ne peut contenir que des caractères alphanumériques et des points et ne doit pas être modifié après sa définition. |
...
|  `result`  |  object  |  Oui  |  Objet qui définit le type d’informations renvoyées par la fonction. Voir l' [Objet de résultat](#result-object) pour plus d’informations. |

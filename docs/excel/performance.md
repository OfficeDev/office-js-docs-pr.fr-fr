...
> [!IMPORTANT]
> De nombreux problèmes de performances peuvent être résolus par l’utilisation recommandée et `sync` les `load` appels. Consultez la section « Améliorations des performances avec les API spécifiques à l’application » des [limites de ressources et de l’optimisation des performances pour les compléments Office](../concepts/resource-limits-and-performance-optimization.md#performance-improvements-with-the-application-specific-apis) pour obtenir des conseils sur l’utilisation efficace des API spécifiques à l’application.
...
Notez que seuls les calculs de formule sont suspendus. Toutes les références modifiées sont toujours reconstruites. Par exemple, renommer une feuille de calcul met toujours à jour toutes les références dans les formules de cette feuille de calcul.
...
L’API JavaScript Excel présente des limitations de taille pour les appels d’API. Excel sur le Web a une limite de taille de charge utile de 5 Mo pour les requêtes et les réponses, et une API retourne une `RichAPI.Error` erreur si cette limite est dépassée. Sur toutes les plateformes, une plage est limitée à cinq millions de cellules pour les opérations d’obtention. Les grandes plages dépassent généralement ces deux limitations.

La taille de charge utile d’une requête est une combinaison des trois composants suivants.

* Nombre d’appels d’API
* Nombre d’objets, tels que des `Range` objets
* Longueur de la valeur à définir ou obtenir

Si une API retourne l’erreur `RequestPayloadSizeLimitExceeded` , utilisez les stratégies de meilleures pratiques décrites dans cet article pour optimiser votre script et éviter l’erreur.

### <a name="strategy-1-move-unchanged-values-out-of-loops"></a>Stratégie 1 : déplacer des valeurs inchangées hors des boucles

Limitez le nombre de processus qui se produisent dans les boucles pour améliorer les performances. Dans l’exemple de code suivant, `context.workbook.worksheets.getActiveWorksheet()` vous pouvez le déplacer hors de la `for` boucle, car il ne change pas dans cette boucle.
...
L’exemple de code suivant montre une logique similaire à l’exemple de code précédent, mais avec une stratégie de performances améliorée. La valeur `context.workbook.worksheets.getActiveWorksheet()` est récupérée avant la `for` boucle, car cette valeur n’a pas besoin d’être récupérée chaque fois que la `for` boucle s’exécute. Seules les valeurs qui changent dans le contexte d’une boucle doivent être récupérées dans cette boucle.
...
Créez moins d’objets de plage pour améliorer les performances et réduire la taille de charge utile. Deux approches pour créer moins d’objets de plage sont décrites dans les sections d’article et les exemples de code suivants.
...
> [!IMPORTANT]
> Utilisez cette stratégie uniquement si vous avez d’abord déterminé que vous dépassez la limite de taille de la demande de charge utile. L’utilisation de plusieurs boucles peut réduire la taille de chaque demande de charge utile pour éviter de dépasser la limite de 5 Mo, mais l’utilisation de plusieurs boucles et de plusieurs `context.sync()` appels a également un impact négatif sur les performances.

L’exemple de code suivant tente de traiter un grand tableau de plages dans une seule boucle, puis un seul `context.sync()` appel. Le traitement d’un trop grand nombre de valeurs de plage dans un `context.sync()` appel entraîne le dépassement de la limite de 5 Mo de la taille de la demande de charge utile.
...
Une autre façon de créer moins d’objets de plage consiste à créer un tableau, à utiliser une boucle pour définir toutes les données de ce tableau, puis à passer les valeurs du tableau à une plage. Cela bénéficie à la fois aux performances et à la taille de la charge utile. Au lieu d’appeler `range.values` chaque plage dans une boucle, `range.values` est une fois appelée en dehors de la boucle.

L’exemple de code suivant montre comment créer un tableau, définir les valeurs de ce tableau dans une `for` boucle, puis passer les valeurs du tableau à une plage en dehors de la boucle.

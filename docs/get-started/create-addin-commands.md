...
**IMPORTANT** Pour les éléments qui contiennent un attribut ID, veillez à fournir un ID unique. Nous recommandons d’utiliser le nom de votre société, ainsi que votre identifiant. Par exemple, utilisez la syntaxe suivante. > <CustomTab id="mycompanyname.mygroupname"> 
...
|**Control** <br/> |Chaque groupe requiert au moins un contrôle. Un élément **Contrôle** peut être un **bouton** ou un **menu**. Utilisez **Menu** pour spécifier une liste déroulante de contrôles. Actuellement, seuls les boutons et les menus sont pris en charge. <br/> Pour plus d’informations, reportez-vous aux sections [Contrôles de bouton](https://dev.office.com/reference/add-ins/manifest/control) et [Contrôles de menu](https://dev.office.com/reference/add-ins/manifest/control). <br/> **REMARQUE** Pour faciliter les opérations de dépannage, nous vous recommandons d’ajouter un élément **Control** et les éléments enfants **Resources** associés un par un.          |

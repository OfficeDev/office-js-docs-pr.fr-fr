...
1. Saisissez `npm run build` dans la console pour générer le projet.
...
1. Pour la propriété `client_id`, remplacez l’espace réservé `{client GUID}` par l’ID d’application que vous avez enregistré lorsque vous avez inscrit le complément. Lorsque vous avez terminé, vous obtenez simplement un GUID entre guillemets simples. Il ne doit pas exister de caractères « {} ».

1. Pour la propriété `client_secret`, remplacez l’espace réservé `{client secret}` par le secret de l’application que vous avez enregistré lorsque vous avez inscrit le complément.
...
1. Remplacez `TODO15` par le code suivant *juste en dessous de la dernière accolade fermante du code que vous avez ajouté à l’étape précédente*. Tenez compte des remarques suivantes à propos de ce code :
...
1. Remplacez `TODO16` par le code suivant *juste en dessous de la dernière accolade fermante du code que vous avez ajouté à l’étape précédente*.
...
1. Ouvrez le fichier \src\auth.ts. Ajoutez la méthode ci-après à la classe `AuthModule`. Tenez compte des informations suivantes :
    ...
    * Le paramètre de ressource est facultatif. Il ne doit pas être utilisé lorsque le [service STS (Secure Token Service)](/previous-versions/windows-identity-foundation/ee748490(v=msdn.10)) est le point de terminaison AAD V2.0. Le point de terminaison V2.0 déduit la ressource des étendues et renvoie une erreur si une ressource est envoyée dans la requête HTTP.
...
1. Remplacez `TODO4` par le code suivant, qui envoie la requête HTTP au point de terminaison de jeton du STS.
...
1. Enregistrez le fichier, mais ne le fermez pas.
...
1. Ajoutez la méthode suivante au bas du fichier. Cette méthode servira la page d’accueil du complément. Le manifeste du complément spécifie l’URL de la page d’accueil.
...
1. Ajoutez la méthode suivante en bas du fichier. Cette méthode traite toutes les requêtes concernant l’API `values`.
...
1. Remplacez `TODO8` par le code suivant. Tenez compte des informations suivantes :
...
1. Remplacez `TODO9` par la ligne suivante. Tenez compte des informations suivantes :
...
1. Créez un partage réseau, ou [partagez un dossier sur le réseau](/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/cc770880(v=ws.11)).

1. Placez une copie du fichier manifeste Office-Add-in-NodeJS-SSO.xml, depuis la racine du projet, dans le dossier partagé.

1. Lancez PowerPoint et ouvrez un document.
...
1. Dans le champ **URL du catalogue**, saisissez le chemin réseau permettant d’accéder au partage de dossier qui contient le fichier Office-Add-in-NodeJS-SSO.xml, puis sélectionnez **Ajouter un catalogue**.

1. Activez la case à cocher **Afficher dans le menu**, puis cliquez sur **OK**.

1. Un message vous informe que vos paramètres seront appliqués lors du prochain démarrage de Microsoft Office. Fermez PowerPoint.
...
1. Si vous n’utilisez pas Visual Studio Code :
   ...
   1. Dans le terminal, entrez **npm run build**.
   1. Ouvrez un second terminal de nœud et accédez au dossier racine du projet.
   1. Dans le terminal, entrez **npm run start**.

1. Si vous utilisez VS Code :
   1. Ouvrez le projet dans VS Code.
   1. Appuyez sur CTRL-MAJ-B pour générer le projet.
   1. Appuyez sur **F5** pour exécuter le projet dans une session de débogage.
...
1. Si l’onglet **Développeur** n’est pas visible sur le ruban, activez-le en procédant comme suit :
   ...
   1. Cliquez sur la case à cocher pour activer **Développeur** dans l’arborescence des noms de contrôle dans la partie droite de la page **Personnaliser le ruban**.
   1. Appuyez sur **OK**.

1. Sous l’onglet **Développeur** de PowerPoint, choisissez **Mes compléments**.

1. Sélectionnez l’onglet **DOSSIER PARTAGÉ**.

1. Choisissez **Échantillon SSO NodeJS**, puis sélectionnez **OK**.

1. Dans le ruban **Accueil**, un nouveau groupe appelé **SSO NodeJS** apparaît avec un bouton intitulé **Afficher le complément** et une icône.
...
1. Cliquez sur le bouton **Afficher le complément** pour ouvrir le complément.

1. Le complément s’ouvre avec une page d’accueil. Cliquez sur le bouton **Obtenir mes fichiers à partir de OneDrive**.

1. Si vous êtes connecté à Office, une liste de vos fichiers et dossiers sur OneDrive apparaîtront en dessous du bouton. La première fois, l’opération peut prendre plus de 15 secondes.

1. Si vous n’êtes pas connecté à Office, une fenêtre contextuelle s’ouvre et vous invite à vous connecter. Une fois que vous êtes connecté, la liste de vos fichiers et dossiers s’affiche après quelques secondes. *N’appuyez pas sur le bouton une deuxième fois.*

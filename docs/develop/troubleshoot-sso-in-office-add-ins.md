...
- [HomeES6.js dans Office-Add-in-ASPNET-SSO](https://github.com/OfficeDev/Office-Add-in-ASPNET-SSO/blob/master/src/Office-Add-in-ASPNET-SSO-WebAPI/Scripts/HomeES6.js)
- [ssoAuthES6.js dans Office-Add-in-NodeJS-SSO](https://github.com/OfficeDev/Office-Add-in-NodeJS-SSO/blob/master/src/public/javascripts/ssoAuthES6.js)
...
- En production, plusieurs causes peuvent entraîner cette erreur. En voici certaines :
    - L’utilisateur dispose d’une identité de compte de service administré Microsoft (MSA).
    - Certaines situations susceptibles d’occasionner d’autres erreurs 13xxx avec un compte professionnel ou scolaire entraînent un erreur 13007 en cas d’utilisation d’un compte MSA.
...
Quelle que soit votre architecture, si la valeur claims a été envoyée à partir d’AAD, `getAccessToken` votre code doit rappeler `authChallenge: CLAIMS-STRING-HERE` et transmettre `options` l’option dans le paramètre. Lorsqu’AAD voit cette chaîne, il demande le(s) facteur(s) supplémentaire(s) à l’utilisateur, puis renvoie un nouveau jeton d’accès qui sera accepté dans le flux « de la part de ».

...
Pour ajouter une nouvelle partie XML personnalisée à un document, utilisez la propriété pour obtenir les parties XML personnalisées du document et appelez la méthode `Document.customXmlParts` [CustomXmlParts.addAsync.](/javascript/api/office/office.customxmlparts#addasync-xml--options--callback-)

...
> [!NOTE]
>
> - Pour plus de clarté, dans cette section, nous appelons le message cible la *page* hôte, mais à proprement parler, les messages sont dirigés vers le *runtime JavaScript* dans le volet Des tâches (ou le runtime qui héberge un fichier de [fonction).](/javascript/api/manifest/functionfile) La distinction n’est significative que dans le cas de la messagerie entre domaines. Pour plus d’informations, consultez [Messagerie inter-domaines au runtime hôte](#cross-domain-messaging-to-the-host-runtime).

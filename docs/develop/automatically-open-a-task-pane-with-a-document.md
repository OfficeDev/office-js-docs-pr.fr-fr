...
|EXCatalog (déploiement via le serveur Exchange) |Le GUID du complément dans le manifeste de complément.|« EXCatalog » La ligne EXCatalog est celle à utiliser avec les compléments qui utilisent le déploiement centralisé dans le centre d’administration Office 365.|La version dans le manifeste de complément.
...
Vous pouvez déployer une version de test de votre complément qui ouvrira automatiquement un volet Office à l’aide du déploiement centralisé via le centre d’administration Office 365. L’exemple suivant montre la façon dont les compléments sont insérés à partir du catalogue de déploiement centralisé à l’aide de la version store d’EXCatalog.

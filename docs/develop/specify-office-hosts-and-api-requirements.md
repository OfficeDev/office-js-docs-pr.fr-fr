...
Pour exécuter votre application dans une application cliente Office, utilisez les éléments et `Hosts` `Host` les éléments du manifeste. Si vous ne spécifiez pas l'élément, votre application s'exécutera dans toutes les applications Office pris en charge par le type spécifié (courrier, volet Office ou `Hosts` `OfficeApp` contenu).

Par exemple, l'exemple suivant et la déclaration spécifient que le add-in fonctionne avec n'importe quelle version d'Excel, qui inclut Excel sur le `Hosts` `Host` web, Windows et iPad.
...
| Nom          | Applications clientes Office                     | Types de add-in disponibles |
|:--------------|:-----------------------------------------------|:-----------------------|
| Base de données      | applications web Access                                | Volet de tâches              |
| Document      | Word sur le web, Windows, Mac, iPad            | Volet de tâches              |
| MailHost      | Outlook sur le web, Windows, Mac, Android, iOS | Courrier                   |
...
- `Methods`L'élément peut contenir un ou plusieurs `Method` éléments. Vous ne pouvez pas utiliser `Methods` l'élément avec les add-ins Outlook.

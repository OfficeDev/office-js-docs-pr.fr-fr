...
7. Remplacez `TODO1` par le code suivant. 
    > [!NOTE]
    > La méthode `ContentControlCollection.getByTag` renvoie un `ContentControlCollection` de tous les contrôles de contenu de la balise spécifiée. Nous utilisons `getFirst` pour obtenir une référence au contrôle souhaité.

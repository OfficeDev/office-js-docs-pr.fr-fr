...
|---|---|
|[Version minimale de l’ensemble des conditions requises pour les boîtes aux lettres](/javascript/office/requirement-sets/outlook-api-requirement-sets)| 1.0|
...
|---|---|---|
|`itemId`| Chaîne|Un ID d’élément mis en forme pour les API REST Outlook|
...
|Condition requise| Valeur|
|---|---|
|[Version minimale de l’ensemble des conditions requises pour les boîtes aux lettres](/javascript/office/requirement-sets/outlook-api-requirement-sets)| 1.3|
|[Niveau d’autorisation minimal](https://docs.microsoft.com/outlook/add-ins/understanding-outlook-add-in-permissions)| Restreint|
|[Mode Outlook applicable](https://docs.microsoft.com/outlook/add-ins/#extension-points)| Composition ou lecture|
...
|Nom| Type| Description|
|---|---|---|
|`itemId`| Chaîne|Un ID d’élément mis en forme pour les services web Exchange (EWS)|
...
|Nom| Type| Description|
|---|---|---|
|`input`| [LocalClientTime](/javascript/api/outlook_1_6/office.LocalClientTime)|Valeur en heure locale à convertir.|
...
|Nom| Type| Description|
|---|---|---|
|`itemId`| Chaîne|L’identificateur des services web Exchange pour un rendez-vous de calendrier existant.|
...
N’utilisez pas la méthode `displayMessageForm` avec un `itemId` qui représente un rendez-vous. Utilisez la méthode `displayAppointmentForm` pour afficher un rendez-vous existant, et `displayNewAppointmentForm` pour afficher un formulaire permettant de créer un nouveau rendez-vous.

##### <a name="parameters"></a>Paramètres :

|Nom| Type| Description|
|---|---|---|
|`itemId`| Chaîne|L’identificateur des services web Exchange pour un message existant.|
...
Affiche un formulaire pour créer un nouveau rendez-vous dans le calendrier.

> [!NOTE]
> Cette méthode n’est pas prise en charge dans Outlook pour iOS ou Outlook pour Android.

La méthode `displayNewAppointmentForm` ouvre un formulaire qui permet à l’utilisateur de créer un nouveau rendez-vous ou une nouvelle réunion. Si des paramètres sont fournis, les champs du formulaire de rendez-vous sont automatiquement remplis avec le contenu des paramètres.
...
| `parameters.location` | Chaîne | Un chaîne contenant le lieu du rendez-vous. La chaîne est limitée à un maximum de 255 caractères. |
...
| `parameters.body` | Chaîne | Le corps du rendez-vous. La contenu du corps est limitée à une taille maximale de 32 Ko. |
...
La méthode `getCallbackTokenAsync` effectue un appel asynchrone pour obtenir un jeton opaque à partir d’Exchange Server qui héberge la boîte aux lettres de l’utilisateur. La durée de vie du jeton de rappel est de 5 minutes.
...
|`userContext`| Objet| &lt;facultatif&gt;|Toute donnée d’état transmise à la méthode asynchrone.|
...
La méthode `getUserIdentityTokenAsync` retourne un jeton que vous pouvez utiliser pour identifier et [authentifier le complément et l’utilisateur avec un système tiers](https://docs.microsoft.com/outlook/add-ins/authentication).

##### <a name="parameters"></a>Paramètres :

|Nom| Type| Attributs| Description|
|---|---|---|---|
|`callback`| fonction||Quand la méthode se termine, la fonction transmise dans le paramètre `callback` est appelée avec un seul paramètre, `asyncResult`, qui est un objet [`AsyncResult`](/javascript/api/office/office.asyncresult).<br/><br/>Le jeton est fourni sous la forme d’une chaîne dans la propriété `asyncResult.value`.|
|`userContext`| Objet| &lt;facultatif&gt;|Toute donnée d’état transmise à la méthode asynchrone.|
...
> 
> Dans ces cas, les compléments doivent plutôt [utiliser des API REST](https://docs.microsoft.com/outlook/add-ins/use-rest-api) pour accéder à la boîte aux lettres de l’utilisateur.

La méthode `makeEwsRequestAsync` envoie une demande EWS à Exchange, de la part du complément. Pour une liste des opérations EWS prises en charge, voir [Appeler des services web depuis un complément Outlook](https://docs.microsoft.com/outlook/add-ins/web-services#ews-operations-that-add-ins-support) .
...
> [!NOTE]
> L’administrateur du serveur doit définir `OAuthAuthentication` à true dans l’annuaire EWS du serveur d’accès client, pour permettre à la méthode `makeEwsRequestAsync` d’effectuer des demandes EWS.
...
|`callback`| fonction||Quand la méthode se termine, la fonction transmise dans le paramètre `callback` est appelée avec un seul paramètre, `asyncResult`, qui est un objet [`AsyncResult`](/javascript/api/office/office.asyncresult).<br/><br/>Le résultat XML de l’appel EWS est fourni comme une chaîne dans la propriété `asyncResult.value`. Si le résultat dépasse 1 Mo en taille, c’est plutôt un message d’erreur qui est renvoyé.|
|`userContext`| Objet| &lt;facultatif&gt;|Toute donnée d’état transmise à la méthode asynchrone.|
...
L’exemple suivant appelle `makeEwsRequestAsync` à utiliser l’opération `GetItem` pour obtenir l’objet d’un élément.

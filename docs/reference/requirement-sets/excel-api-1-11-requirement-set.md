...
| [Résolution](../../excel/excel-add-ins-comments.md#resolve-comment-threads) des commentaires | Résoudre les threads de commentaires et obtenir l’état de résolution. | [Commentaire](/javascript/api/excel/excel.comment) |
...
Le tableau suivant répertorie les API dans l’ensemble de conditions requises de l’API JavaScript pour Excel 1,11. Pour afficher la documentation de référence de l’API pour toutes les API prises en charge par l’ensemble de conditions requises de l’API JavaScript pour Excel 1,11 ou antérieure, voir [API Excel dans l’ensemble de conditions requises 1,10 ou version antérieure](/javascript/api/excel?view=excel-js-1.11).
...
||[id](/javascript/api/excel/excel.commentmention#id)|ID de l’entité. L’ID correspond à l’un des ID `CommentRichContent.richContent`dans.|
...
|[Feuille de calcul](/javascript/api/excel/excel.worksheet)|[onRowHiddenChanged](/javascript/api/excel/excel.worksheet#onrowhiddenchanged)|Survient lorsque l’état masqué d’une ou plusieurs lignes a été modifié sur une feuille de calcul spécifique.|
